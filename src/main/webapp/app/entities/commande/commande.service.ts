...
export class CommandeService {
  ...
  protected convertDateFromClient(commande: ICommande): ICommande {
    const copy: ICommande = Object.assign({}, commande, {
      dtexecution: commande.dtexecution && commande.dtexecution.isValid() ? commande.dtexecution.format(DATE_FORMAT) : undefined,
    });
    return copy;
  }
  ...
  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.dtexecution = res.body.dtexecution ? moment(res.body.dtexecution) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((commande: ICommande) => {
        commande.dtexecution = commande.dtexecution ? moment(commande.dtexecution) : undefined;
      });
    }
    return res;
  }
